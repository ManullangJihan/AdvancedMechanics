...
# ╔═╡ f0105df8-b59e-11ee-21c1-e5dfa0b65dd6
begin
	using PlutoUI
	using LinearAlgebra
...
# ╔═╡ 284ea428-ef5c-4dea-8d75-bd5a14fb073c
md"# Solve Equations of Motion for Baton Thrown into Air"

# ╔═╡ 3497f039-a0f2-4b31-8136-a4c7d7bd314a
md"### Import Libraries"

# ╔═╡ 4be9f2f3-a143-4f28-a223-a7eda9b8d8d3
md"### Write the ODE of the System"

# ╔═╡ 8e1d21ce-b7c9-4c01-9ca0-567af1a42fbc
md"
### Set System Parameters
m1:\
$(@bind m1 NumberField(0.1:10.0, 0.1))\
m2:\
$(@bind m2 NumberField(0.1:10.0, 0.1))\
L:\
$(@bind L Slider(1.0:10.0, 1.0, true))\
"

# ╔═╡ c451bc2b-c86c-4158-853b-34f8d3393107
begin
	const g = 9.81
...
# ╔═╡ 605cc2b2-13bc-42a1-807c-d8c6d60847d9
md"### Set Simulation Parameter"

# ╔═╡ 40050ecc-701c-46e5-9863-9cbe9fdba8c5
md"
End Simulation Time:\
$(@bind t_end Slider(1.0:20.0, 4.0, true))\
x₀: $(@bind x0 Slider(0.0:5.0, 0.0, true))\
Vx₀: $(@bind Vx0 Slider(0.0:5.0, 4.0, true))\
y₀: $(@bind y0 Slider(0.0:10.0, L, true))\
Vy₀: $(@bind Vy0 Slider(0.0:30.0, 20.0, true))\
θ₀: $(@bind theta0 NumberField(-2π:2π, 1.57))\
θ̇₀: $(@bind omega0 Slider(0.0:5.0, 2.0, true))\
"

# ╔═╡ 756df1de-64cc-405b-9555-c7666d2b8790
begin
	tspan = (0.0, t_end)
	# t = tspan[1]:0.1:tspan[2]
	u0 = [x0, Vx0, y0, Vy0, theta0, omega0]
...
# ╔═╡ 82223cac-89b0-4d43-a05d-43ebdf4f6719
md"### Visualize the Result"

# ╔═╡ 0315925a-0d3f-48c4-9aca-417a056a209f
begin

@gif for j = 1:length(t)
    theta = arr_sol[5, j]
    X = arr_sol[1, j]
    Y = arr_sol[3, j]
    xvals = [X, X+L*cos(theta)]
    yvals = [Y, Y+L*sin(theta)]
    plot(xvals, yvals, legend=false, xlims=(0.0, 100.0), ylims=(0.0, 100.0))
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "8429dcca657a172c55f0a4cfbe7453f5137a1d29"
